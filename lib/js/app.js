...
;angular.module('portfolio').controller('projectsCtrl', function(Projects, $state) {
  ...
  function bgDraw() {
    ...
    for (var x = 0; x < bgWidth; x++) {
      ...
      if (mouseMove) {
        ...
        yMouse = 1.5 * ampMouse * Math.cos( (x - mouseMoveEvent.clientX * (bgWidth / window.innerWidth) ) * (Math.PI / 180) / 2 );
      }
  ...
  $('button.navLinks').hover(
       function(){ // mouseenter
         $('.linkItems').addClass('active')
       }
  )

  $('body').on('mousemove', function(event) {
    mouseMoveEvent = event;
    if (mouseMoveEvent.clientX < (window.innerWidth - 200) || mouseMoveEvent.clientY > (200)) { // Hiding nav links
